...
#let _tblock(self: none, title: none, it) = {
  grid(
    ...
    block(
      ...
      text(fill: self.colors.neutral-lightest, title),
    ),
  ...
  self = utils.merge-dicts(
    ...
    config-page(
      ...
      background: place(
        ...
        dx: 180pt,
        dy: 25pt,
        image("pic/logo-shallow-red.svg", height: 93%),
      ),
      footer: none,
    ),
  )
...
#let bib-slide(
  ...
  show selector(bibliography): it => {
    show regex("\[\d+\]"): num => {
      move(dy: -0.5em, image("pic/beamericonarticle.svg", height: 1.5em))
    }
    it
  }
  touying-slide(self: self, config: config, std.align(horizon, body))
})
  ...
  let header(self) = {
    ...
    grid(
      ...
      grid(
        ...
        align: horizon,
        utils.call-or-display(self, self.store.navigation),
        rect(fill: self.colors.primary, image("pic/logo-short.svg", height: 100%), height: 1.85em, inset: 5pt),
      ),
  ...
  show: touying-slides.with(
    config-page(
      ...
      background: place(
        ...
        dx: 180pt,
        dy: 25pt,
        image("pic/logo-grey.svg", height: 93%),
      ),
    ),
    config-common(
      slide-fn: slide,
